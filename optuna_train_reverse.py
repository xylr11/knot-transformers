...
from data.loader import get_dataloaders_reverse
from utils.config import load_config, get_class
from utils.plot_batch_pred_vs_actual import plot_batch_pred_vs_actual

def objective(trial, config_path="config/optuna_default_reverse.json", save_dir="states/"):
    ...
    train_loader, val_loader, _ = get_dataloaders_reverse(
...
def run_optuna(config_path="config/optuna_default_reverse.json", save_dir="states/", n_trials=20):
    ...
    _, _, test_loader = get_dataloaders_reverse(batch_size=config["train_params"]["batch_size"])
    with torch.no_grad():
        for x_tensor, x_mask, y_tensor in test_loader:
            x_tensor, x_mask, y_tensor = x_tensor.to(device), x_mask.to(device), y_tensor.to(device)
            pred_tensor, _ = model(x_tensor, x_mask)
            plot_batch_pred_vs_actual(pred_tensor, y_tensor, plot_path=f"plots/best_trial_pred_vs_actual_reverse_{study.best_trial.number}.png", n=8)
            break


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="config/optuna_default_reverse.json")
